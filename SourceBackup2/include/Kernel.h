	...
	*/
	namespace IO
	{
		...
		namespace Driver
		{
			...
			typedef int(*Driver_Func)(DRIVER *, unsigned int argc, void **argv);

			// Basically just read from ELF
			struct DRIVER
			{
				Driver_Func *vectorTable;
				unsigned int tableSize;

				Buffers::Buffer _stdout;
				Buffers::Buffer _stdin;
				Buffers::Buffer _stderr;
			};
		...
		*/
		namespace SDMMC
		{
			...
			// getFileData(const char *fileName)
			// getFileData(char *fileName)
			// writeFile(const char *fileName, mem_t *data, unsigned int fileSize)
			// writeFile(char *fileName, mem_t *data, unsigned int fileSize)
			// 
			// (int:len) getAbsPath(char *relPath, char *relTo, char **result)

...
LuaProcessor *BBP::Services::Lexer::P_Create(bool link, bool strip, HALFWORD a, HALFWORD b)
{
	...
	proc->extVarTable = new BBP::IO::Buffers::stringTable();
	proc->intVarTable = new BBP::IO::Buffers::stringTable();
	proc->varTypeTable = new BBP::IO::Buffers::stringTable();
	proc->funcSigTables = new BBP::IO::Buffers::stringTable();
	proc->funcBodyTables = new BBP::IO::Buffers::stringTable();
	proc->globalsTable = new BBP::IO::Buffers::stringTable();
	proc->fileTable = new BBP::IO::Buffers::stringTable();
...
{
	BBP::IO::Buffers::setStringTableSize(proc->extVarTable, 0);
	BBP::IO::Buffers::setStringTableSize(proc->intVarTable, 0);
	BBP::IO::Buffers::setStringTableSize(proc->varTypeTable, 0);
	BBP::IO::Buffers::setStringTableSize(proc->funcSigTables, 0);
	BBP::IO::Buffers::setStringTableSize(proc->funcBodyTables, 0);
	BBP::IO::Buffers::setStringTableSize(proc->globalsTable, 0);
	BBP::IO::Buffers::setStringTableSize(proc->fileTable, 0);
...
void BBP::Services::Lexer::P_EmitELFLinkInfo(LuaProcessor *proc)
{
	...
	BBP::IO::Buffers::stringTable *externalSymbolTable = BBP::IO::Buffers::createStringTable();
	...
	for (unsigned int sym = 0; sym < proc->intVarTable->stringCount; sym++)
	{
		...
		BBP::IO::Buffers::addStringToTable(externalSymbolTable, proc->extVarTable->strings[sym]);
	...
	BBP::Services::Binaries::ELF::sectionBuilder *relocationStrings = BBP::Services::Binaries::ELF::createSection(".exp.str", BBP::Services::Binaries::ELF::emitStringTable(externalSymbolTable), SHT_STRTAB, BBP::Services::Binaries::ELF::calculateStringTableLength(externalSymbolTable), true);
	...
	BBP::IO::Buffers::setStringTableSize(externalSymbolTable, 0);
...
void BBP::Services::Lexer::P_EmitELFFunctions(LuaProcessor *proc)
{
	...
	unsigned char *funcBodyData = BBP::Services::Binaries::ELF::emitStringTable(proc->funcBodyTables);
	BBP::Services::Binaries::ELF::sectionBuilder *funcBodySection = BBP::Services::Binaries::ELF::createSection(".text.func", funcBodyData, SHT_PROGBITS, BBP::Services::Binaries::ELF::calculateStringTableLength(proc->funcBodyTables), true);
	...
	for (unsigned int func = 0; func < proc->funcSigTables->stringCount; func++)
	{
		...
		// Keep track if undefined
		bool defined = true;

		// Remove any '(' if present
		char *name = proc->funcSigTables->strings[func];
		for (unsigned int k = 0; name[k]; k++)
		{
			if (name[k] != '(')
				continue;
			name[k] = 0;
		...
		unsigned int size = 0;
		if (defined)
			size = proc->funcBodyTables->lengths[func];
		else
			size = 0;
		size = symb->st_size;
		...
		addELFSymbol(proc->elf, symb, name);
		runningLength += size;
	}

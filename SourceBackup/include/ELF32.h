...
#error "Unsupported ELF32. Please use ELF64"
...
typedef struct {
	...
	ADDRESS			e_entry; 		///< Entry point address for executables, Constructor address for shared libraries, otherwise 0.
	OFFSET			e_phoff; 		///< Program Header Offset (Relative to File start)
	OFFSET			e_shoff; 		///< Section Header Offset (Relative to File start)
	WORD			e_flags; 		///< Flags
...
typedef struct {
	...
	WORD			sh_flags = 0; 		///< Flags. 0x1 - Writeable, 0x2 - part of the sections that are loaded into memory, 0x4 - executable.
	ADDRESS			sh_addr = 0; 		///< ?
	OFFSET 			sh_offset = 0; 		///< The offset relative to the beginning of the file where this segment is.
	WORD 			sh_size = 0;  		///< How large this section is in the file
	...
	WORD			sh_info = 0; 		///< Info?
	WORD 			sh_addralign = 0; 		///< ?
	WORD 			sh_entsize = 0; 		///< Size of each entry?
} SectionHeader;
...
typedef struct {
	...
	WORD			st_size = 0; 		///< The size of whatever this symbol is (for example the size of 'main')

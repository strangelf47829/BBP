...
void BBP::Services::Lexer::L_moveToEndOfWord(lex_context *context)
{
	// Increments "context->atLocation" until a whitespace is found
	for (; !L_isWhitespace(context, context->atLocation); context->atLocation++);
}

void BBP::Services::Lexer::L_moveToStartOfNextWord(lex_context *context)
{
	// If we are inside a word, move until end of word
	if (!L_isWhitespace(context, context->atLocation))
		L_moveToEndOfWord(context);
	// Move until we are not in white space
	for (; L_isWhitespace(context, context->atLocation); context->atLocation++);
}
	...
	while (context->atLocation < context->dataSize)
	{
		...
		for (kwIndex = 0; kwIndex < context->keywordCount; kwIndex++)
		{
			...
			if ((character == ' ' || character == '\n' || character == '\t' || character == '\r' || character == 1) && !context->isInLiteralContext)
			{
				...
				// Continue the while loop if we know we are going to return new line
				if (!((character == '\n' || character == '\r') && context->careAboutNewLine))
					goto switch_whilecontinue;
				else
			...
			// Return new line if action is needed
			if ((character == '\n' || character == '\r') && context->careAboutNewLine)
				return context->newline;
			...
			*index = context->atLocation - context->words[kwIndex].length /*Pesky off-by-one!*/;
...
{
	char tmp = context->data[end + 1];
	context->data[end + 1] = 0;
